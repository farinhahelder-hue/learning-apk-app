  ...
  void _answer(String answer) {
    ...
    setState(() {
      ...
      if (correct) {
        ...
        _lastFeedback = _combo >= 3
            ...
            : AppConstants.encouragements[
                DateTime.now().millisecond % AppConstants.encouragements.length];
      } else {
        _combo = 0;
    ...
    Future.delayed(const Duration(milliseconds: 900), () {
      ...
      if (_current < _exercises.length - 1) {
        setState(() { _current++; _lastFeedback = null; });
      } else {
        _finish();
      }
    });
        ...
        actions: [
          if (!_finished)
            Padding(
              ...
              child: Row(
                children: [
                  Icon(Icons.timer, color: _timeLeft <= 10 ? Colors.red : widget.color),
        ...
        children: [
              ...
              if (_combo >= 2)
                Container(
                  ...
                  child: Text('🔥 Combo x$_combo',
                      style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.deepOrange)),
          ...
          if (_lastFeedback != null)
            Container(
              ...
              decoration: BoxDecoration(
                color: _lastCorrect == true ? const Color(0xFFE8F5E9) : const Color(0xFFFFEBEE),
            ...
            ).animate().fadeIn(duration: 200.ms).scale(duration: 300.ms, curve: Curves.elasticOut),
          // Question
          Container(
            width: double.infinity,
          ...
          ...ex.options.asMap().entries.map((e) {
            ...
            ).animate(delay: Duration(milliseconds: 60 * e.key)).fadeIn().slideX(begin: 0.1);
          }),
        ],
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              stars == 3 ? 'Parfait Emilie ! Tu maîttrises ce chapitre !' :
              stars == 2 ? 'Très bien ! Encore un peu de pratique 💪' :
              stars == 1 ? 'Bien essayé ! Réessaie pour progresser !' :
            ...
            ElevatedButton(
              onPressed: () {
                setState(() {
                  _current = 0; _score = 0; _combo = 0; _timeLeft = 60;
                  _finished = false; _lastFeedback = null;
                  _exercises.shuffle();
                  _session = GameSession(
                    worldId: widget.worldId, skillId: widget.skillId,
                    gameType: GameType.arcade, exercises: _exercises,
                  );
                });
                _startTimer();
              },
